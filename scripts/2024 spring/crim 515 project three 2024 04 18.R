...
# Step 2 3: connect to working directory for data, get data, and clean data
...
counts.word <- counts.my.clean.words
...
### multiple graphs, for one article (with sentiment)
my.clean.article <- subset(my.sentiment, file == "lum et al 2023.pdf") #yep we’re testing mine
counts.sentiment.my.clean.article <- my.clean.article %>% count(word, sentiment, sort = TRUE)
top.my.clean.article <- subset(counts.sentiment.my.clean.article, 
                               n >= quantile(counts.sentiment.my.clean.article$n, 0.75))
wordpairs.article <- my.clean.article %>% pairwise_count(word, file.ID, sort = TRUE)
ggplot(top.my.clean.article, aes(x= reorder(word, n), y=n, fill= sentiment)) + 
  geom_bar(stat = "identity") + 
  coord_flip()
my.counts <- counts.sentiment.my.clean.article[c(1,3)]
names(my.counts) <- c("word", "freq")
wordcloud2(data = my.counts, size = 1.0, 
           color = rep_len(c("green","blue","red"), nrow(my.counts)), 
           shape = circle)
set.seed(611)
...
reviewLDA <- LDA(reviewDTM, k = 13, control = list(seed = 347)) # experiment with ‘10’ as the “right” number of topics

...
The goal of this project is **forecast** future **calls for service** activity in the **City of Fairfax, Virginia** for calendar year 2025. 
...
```{r}
calls <- read.csv(sprintf("https://docs.google.com/uc?id=%s&export=download", "1gniFP35GEfxycgpKGHAzkgSYmjTteTKm"))
```

3. Determine the total count and percentage of each call type:
```{r}
sum.calls <- calls %>%
  group_by(type) %>%
...
4. Filter the call types to forecast:
```{r}
group <- c("TRESPASSING", "TRAFFIC STOP")
calls.subset <- calls %>% filter(calls$type %in% group)
```

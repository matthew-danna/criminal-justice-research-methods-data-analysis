...
ggplot(data = world) + 
  geom_sf() + 
  geom_sf(data = states, fill = NA) + 
  geom_point(data = wapo.data.clean, aes(x = longitude, y = latitude), size = 2, shape = 1, fill = "darkred") + coord_sf(xlim = c(-135, -60), ylim = c(25, 50), expand = FALSE) + 
...
# Step 5: Census time!
census_api_key("794f37c27092be60b131e4f207abcf950f227f38", install = TRUE) # run once!
...
temp.race.population <- race.2022 %>% 
...
race.total$PCT <- race.total$estimate/sum(totalpop.2022$Population)*100

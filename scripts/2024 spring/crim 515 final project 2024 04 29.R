...
arrests$hour <- substr(arrests$time, 0, 2)
...
calls$hour <- substr(calls$time, 0, 2)
...
crashes$hour <- substr(crashes$time, 0, 2)
...
unique(calls$type)
unique(crime$crime.code.description)
unique(crashes$type)
...
##### next steps
# OPTIONAL t test: 
### counts of events by week or year.month with a date that matters for a breakpoint
### potential break-points could be based on construction/development in the area

# hotspots: 
### transparent points, densities, or contours. 
### facet by DOW, Hour, event type, year, month

# changes over time: based on your summary tables, maybe t test, graphs, and maps
### identify any specific changes

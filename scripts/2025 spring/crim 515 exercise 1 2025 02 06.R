...
calls %>% filter(hour != "NA") %>%
  ggplot() +
  geom_line(aes(x = as.factor(hour)), stat = "count", group = 1, 
            color = "blue", size = 1) +
  labs(title = "Calls by Hour Reported \n2007 - 2024", 
       x = "Hour Reported", y = "Number of Calls (Thousands)") +
...
calls %>% filter(year != "NA") %>%
  ggplot() +
  geom_bar(aes(x = as.factor(year), fill = call.type), 
           stat = "count") +
  labs(title = "Calls by Year \n2007 - 2024", x = "Year", 
       y = "Number of Calls (Thousands)", fill = "Call Type") +
  theme(plot.title = element_text(hjust = 0.5, size = 14),
        axis.text.x = element_text(color = "black", size = 10),
        axis.text.y = element_text(color = "black", size = 10),
        axis.ticks.y = element_blank(),
